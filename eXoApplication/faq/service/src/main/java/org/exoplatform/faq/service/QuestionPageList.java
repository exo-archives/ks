...
public class QuestionPageList extends JCRPageList {
	...
	/*
	 * private Boolean[] ValuesToBoolean(Value[] Val) throws Exception { if(Val.length < 1) return new Boolean[]{} ; Boolean[] bools = new Boolean[Val.length] ; for(int i = 0; i < Val.length; ++i) {
	 * bools[i] = Val[i].getBoolean(); } return bools; }
	 */
	/**
