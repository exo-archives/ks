...
public class UIUserWatchManager extends UIFormTabPane implements UIPopupComponent {
	...
	/*
	 * static public class ChangeTabActionListener extends EventListener<UIUserWatchManager> { public void execute(Event<UIUserWatchManager> event) throws Exception { UIUserWatchManager watchManager =
	 * event.getSource() ; int id = Integer.parseInt(event.getRequestContext().getRequestParameter(OBJECTID)); watchManager.tabSelect = id;
	 * event.getRequestContext().addUIComponentToUpdateByAjax(watchManager) ; } }
	 */
