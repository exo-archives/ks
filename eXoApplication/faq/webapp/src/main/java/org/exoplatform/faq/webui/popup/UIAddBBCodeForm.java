...
@ComponentConfig(
		...
		template = "app:/templates/faq/webui/popup/UIAddBBCodeForm.gtmpl",
		events = {
  ...
  private String getReplaceByBBCode() throws Exception {
	...
	static	public class SaveActionListener extends EventListener<UIAddBBCodeForm> {
		public void execute(Event<UIAddBBCodeForm> event) throws Exception {
			...
			if(FAQUtils.isFieldEmpty(description)) description = " ";
			...
			try {
				uiForm.listBBCode.addAll(uiForm.faqService.getAllBBCode());
	    } catch (Exception e) {
	    }
    	for (BBCode code : uiForm.listBBCode) {
	      if(uiForm.bbcode.getTagName().equals(code.getTagName()) && (uiForm.bbcode.isOption() == code.isOption()) &&
			...
			try {
				...
				uiForm.faqService.saveBBCode(bbcodes);
      } catch (Exception e) {
	      e.printStackTrace();
      }
      UIWatchContainer popupContainer = uiForm.getAncestorOfType(UIWatchContainer.class) ;
			UIPopupAction popupAction = popupContainer.getChild(UIPopupAction.class);
			popupAction.deActivate() ;
			UIBBCodeManagament codeManagament = popupContainer.findFirstComponentOfType(UIBBCodeManagament.class);
			codeManagament.setListBBcode();
			popupContainer.getChild(UISettingForm.class).setCheckBoxBBCode(codeManagament.getListBBcode());
	...
	static	public class CancelActionListener extends EventListener<UIAddBBCodeForm> {
		public void execute(Event<UIAddBBCodeForm> event) throws Exception {
			...
			UIWatchContainer popupContainer = addBBCodeForm.getAncestorOfType(UIWatchContainer.class) ;
			UIPopupAction popupAction = popupContainer.getChild(UIPopupAction.class);
			popupAction.deActivate() ;
