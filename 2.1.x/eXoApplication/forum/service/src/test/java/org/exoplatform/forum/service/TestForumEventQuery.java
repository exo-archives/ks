...
public class TestForumEventQuery extends TestCase {
	...
	public void testQuerySearchCategory() {
  	...
  	eventQuery.setFromDateCreated(calendar);
  	predicate += " and ((@exo:createdDate >= xs:dateTime('" + ISO8601.format(calendar) 
  						+ "')) and (@exo:createdDate <= xs:dateTime('" + ISO8601.format(calendar) + "'))) ";
  	assertEquals(selector + predicate + "]", eventQuery.getPathQuery(categoryIds));
 ...
 // only forum name
  	...
  	eventQuery.setFromDateCreated(calendar);
  	predicate = tempPredicate + " and ((@exo:createdDate >= xs:dateTime('" + ISO8601.format(calendar) + "')) " +
  							"and (@exo:createdDate <= xs:dateTime('" + ISO8601.format(calendar) + "'))) ";
  	assertEquals(selector + predicate + "]", eventQuery.getPathQuery(categoryIds));
   ...
   // only forum name
    	...
    	predicate += " and (@exo:isApproved='true' and @exo:isActive='true' and @exo:isWaiting='false' and @exo:isActiveByForum='true')" +
    							 " and (not(@exo:canView) or @exo:canView='' or @exo:canView='john' or @exo:canView='/exo' or @exo:canView='/exo:moderator' or @exo:canView=' ')";
    	assertEquals(selector + predicate + "]", eventQuery.getPathQuery(categoryIds));
